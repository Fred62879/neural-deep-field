...
class LatentQuantizer(nn.Module):
    ...
    def __init__(self, latent_dim, num_embd, mlp_cho, mlp_args, output_scaler,
        ...
        if self.quantize:
            self.quantization = Vector_Quantizer \
                (latent_dim, num_embd, beta, calculate_loss, cdbk_seed)
    ...
    def forward(self, input):
        ...
        if self.quantize:
            latents_q, cdbk_loss, embd_ids = self.quantization([latents, None])
        else: latents_q, cdbk_loss, embd_ids = latents, None, None
        return (latents_q, latents, scaler, redshift, cdbk_loss, embd_ids)
...
class LatentQuantizer(nn.Module):
    ...
    '''
    def __init__(self, latent_dim, num_embd, beta, calculate_loss, seed):
        super(LatentQuantizer, self).__init__()
        ...
        self.init_codebook(seed)

    def init_codebook(self, seed):
        torch.manual_seed(seed)
        self.codebook = nn.Embedding(self.latent_dim, self.num_embd)
        self.codebook.weight.data.uniform_ \
            (-1.0 / self.latent_dim, 1.0 / self.latent_dim)
        #self.codebook.weight.data *= 50
        #self.codebook = torch.zeros(self.latent_dim, self.num_embd).to('cuda:0')
    ...
    def quantize(self, z):
        ...
        min_embd_ids = find_closest_tensor(z_f, self.codebook.weight) # [bsz]
        #min_embd_ids = find_closest_tensor(z_f, self.codebook) # [bsz]
        #print(len(list(set(min_embd_ids.detach().cpu().numpy()))))

        # replace each z with closest embedding
        encodings = one_hot(min_embd_ids, self.num_embd) # [n,num_embd]
        encodings = encodings.type(z.dtype)
        z_q = torch.matmul(encodings, self.codebook.weight.T).view(z_shape)
        #z_q = torch.matmul(encodings, self.codebook.T).view(z_shape)
        return z_q, min_embd_ids
    ...
    def forward(self, input):
        ...
        z_q, min_embd_ids = self.quantize(z)
